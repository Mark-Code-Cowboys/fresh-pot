...
final entitlementServiceProvider = Provider<EntitlementService>((ref) {
  final service =
      StoreEntitlementService(ref.watch(kvStoreProvider), fpStoreProducts);
...
/// Bags ever added on this device, live. Deleting one doesn't lower
/// it: the free tier is spent by adding, not by keeping — a bag is
/// tasting history, not a recyclable slot.
final lifetimeBeansProvider = StreamProvider<int>(
  (ref) => ref.watch(beanRepositoryProvider).watchLifetimeCreated(),
);
...
final freeTierUsageProvider = Provider<FreeLimitUsage?>((ref) {
  ...
  final count = ref.watch(lifetimeBeansProvider).value;
  if (pro == null || pro || count == null) return null;
  return beanFreeLimit.usage(count);
});
